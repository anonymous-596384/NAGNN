...
def discriminatorLoss(options, variablesMap, logits, lbl_in, lbl_in_1, isSupervised_flag, isReal_flag, isOnlyAddZerosCol_flag, isDLoss_flag, isPreTrain_flag):    
    ...
    discriminator loss
    """
    log_resh = tf.reshape(logits, [-1, options['all_class_num']]) # prediction
    lab_resh = tf.reshape(lbl_in, [-1, options['all_class_num']]) # groundtruth
    lab_resh_1 = tf.reshape(lbl_in_1, [-1, options['all_class_num']]) 
    msk_in = tf.ones(shape=tf.shape(lab_resh)[0], dtype=tf.bool)
    msk_resh = tf.reshape(msk_in, [-1]) 
    
    loss_un = masked_softmax_cross_entropy_unsupervised(log_resh, msk_resh, isReal_flag, options['discountCE'])
    loss_su = masked_softmax_cross_entropy_supervised(log_resh, lab_resh, msk_resh, options['discountCE'])
    loss_su_1 = masked_softmax_cross_entropy_supervised(log_resh, lab_resh_1, msk_resh, options['discountCE'])
...
def masked_softmax_cross_entropy_unsupervised(logits, mask, isReal_flag, discountCE):
    """
    softmax cross entropy for unsupervised
    """
    logits = tf.nn.softmax(logits)
    off = 1e-6 
    loss_r = -tf.log(tf.reduce_sum(logits[:, :-1], axis=1) + off) * discountCE 
    loss_f = -tf.log(logits[:, -1] + off) * discountCE 
...
def masked_softmax_cross_entropy_supervised(logits, labels, mask, discountCE):
    """
    softmax cross entropy for unsupervised
    """
    loss = tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=(tf.cast(labels, tf.float32) * discountCE))
    mask = tf.cast(mask, dtype=tf.float32)
    mask /= tf.reduce_mean(mask)
    loss *= mask
    return tf.reduce_mean(loss)
